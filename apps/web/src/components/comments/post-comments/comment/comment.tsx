...
import { useIntersectionRatio } from "../../../../hooks";
import {
  classNames,
  getAvatarUrl,
  isElementInViewport,
...
export const Comment = memo<Props>(
  ({ comment, level = 1, replyTo, onReplyToChange }) => {
    const ref = useRef<HTMLDivElement>(null);
    const [isQueryEnabled, setIsQueryEnabled] = useState(false);
    // disable updates when comment is not displayed on user's screen
    const intersectionRatio = useIntersectionRatio(ref, { threshold: 1 });
    const childrenQuery = trpc.comments.getByPost.useQuery(
      ...
      {
        enabled: isQueryEnabled && intersectionRatio > 0,
    ...
    useLayoutEffect(() => {
      ...
      if (router.query.comment === comment.id.toString()) {
        ref.current.scrollIntoView({ behavior: "smooth", block: "center" });
      }
    }, [comment.id, router.query.comment]);
    ...
    return (
      ...
      >
        ...
        <div
          className={classNames({
            "flex flex-col gap-y-1 -mx-4 px-4 py-2 transition-colors duration-300":
              true,
            "bg-amber-100": router.query.comment === comment.id.toString(),
          })}
        >
        ...
        {childrenQuery.isLoading ? (
          ...
          (children?.length ?? 0) !== comment._count.children && (
            <div>
              <button
